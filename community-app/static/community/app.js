...
function quizStampTag(quiz){
 const icon=passport.find(item=>item.id===quiz.stamp);
 return icon?`<a class="quiz-stamp-tag" href="#passport" title="${escapeHTML(icon.label)}" aria-label="${escapeHTML(icon.label)}">${stampIcon(icon.id)}</a>`:'';
}

...
function result(){app.innerHTML=translateHTML(`<section class="quiz-shell question result"><div class="eyebrow">SFIDA COMPLETATA</div><h1>Una scoperta tira l’altra.</h1><div class="score">${points}<small> / 30 punti</small></div><p>Hai risposto correttamente a ${points/10} domande su 3.<br>Ogni risposta è un’occasione per imparare.</p>${resultStamp()}<button class="primary" id="more">Esplora altri quiz →</button> <button class="outline" id="path">Il mio percorso</button><p class="note">${name?"Risultato salvato nel tuo profilo.":"Accedi o registrati per conservare questo risultato."}</p></section>`);document.querySelector('#more').onclick=()=>location.hash='home';document.querySelector('#path').onclick=()=>location.hash='profile'}
function profile(){current=null;app.innerHTML=translateHTML(`<section class="profile-head"><div class="eyebrow">IL MIO PERCORSO</div><h1>${name?'Ciao, '+escapeHTML(name)+'.':'La tua curiosità, in viaggio.'}</h1><p>${name?"Qui ritrovi i tuoi migliori risultati salvati.":"Qui ritrovi i risultati di questa sessione. Accedi per conservarli."}</p><div class="stats"><div class="stat"><strong>${total()}</strong><span>Punti raccolti</span></div><div class="stat"><strong>${Object.keys(completed).length} / ${quizzes.length}</strong><span>Quiz completati</span></div><div class="stat"><strong>${Object.keys(completed).length?'Esploratore':'Si parte!'}</strong><span>Il tuo percorso</span></div></div><p><a class="text-link" href="#passport">Apri il tuo passaporto →</a></p><div class="section-head"><h2>Le tue scoperte</h2><a href="#home">Esplora i quiz ↗</a></div>${Object.keys(completed).length?Object.keys(completed).map(id=>`<div class="history"><strong>${quizzes.find(q=>q.id===id).title}</strong><span>${completed[id]} / 30 punti</span></div>`).join(''):'<div class="empty"><h3>Il primo indizio ti aspetta.</h3><p>Completa un quiz e vedrai qui il tuo risultato.</p><a href="#home" class="text-link">Scegli un quiz →</a></div>'}<div class="strip"><div><strong>${name?'Il tuo profilo: '+escapeHTML(name):'Dai un nome al tuo percorso.'}</strong><p>${name?"I punti totali sommano il risultato migliore di ogni quiz.":"Registrati con email, nickname e password. L’email resta privata."}</p></div><button class="outline" id="profile-auth">${name?'Esplora altri quiz':'Accedi o registrati'} →</button></div></section>`);document.querySelector('#profile-auth').onclick=()=>name?location.hash='home':openAuth()}
